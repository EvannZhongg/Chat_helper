...
import { shallow } from 'zustand/shallow'; // 导入 shallow，用于确保选择器稳定

function AnalysisPage() {
  const { profileId } = useParams();

  // 1. 使用单一选择器 + shallow 提取所有状态和 actions
  const {
    currentProfile, getProfile, isLoading,
    insights, fetchInsights, isInsightLoading,
    analysisDateRange, fetchDateRange,
    triggerFullAnalysis, isPersonaLoading,
    analysisProgress
  } = useProfileStore(
    state => ({
      ...
      isInsightLoading: state.isInsightLoading,
      analysisDateRange: state.analysisDateRange,
      fetchDateRange: state.fetchDateRange,
      triggerFullAnalysis: state.triggerFullAnalysis,
      isPersonaLoading: state.isPersonaLoading,
      analysisProgress: state.analysisProgress,
    }),
    shallow // <-- 使用 shallow 确保只有值改变时才重渲染
  );
  ...
  }, [profileId, currentProfile, getProfile]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentProfile, profileId, fetchDateRange, fetchInsights]);
